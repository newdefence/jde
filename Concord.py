...
def read_sheet1(sheet, fields1, fields2, sum_callback, field2_callback):
...
def read_invoice(sheet):
    ...
    def fn_all_sum(row, cols):
        all_sum.update({
            'row': row, 'errors': set(),
            '总数量': value(row[cols['总数量']]),
            '总合计': value(row[cols['总合计']]),
            '总毛重': value(row[cols['总毛重']]),
            # '总件数': value(row[columns['总件数']]),
            # NOTE: 总件数，总净重没有
        })
    def fn_row(detail):
        if detail['合计'] != detail['数量'] * detail['单价']:
            detail['errors'].add('合计计算错误')

    invoices, columns = read_sheet1(sheet, ('发票号', 'po号', '物料号'),
                                    ('数量', '单价', '合计', '总数量', '总合计', '总件数', '总毛重'), fn_all_sum, fn_row)
    all_invoices = invoices.values()
    for invoice in all_invoices:
        sum1, details = invoice['sum'], invoice['details']
        total_qty, total_invoice_value, total_gross_weight, total_pkgs = \
            sum1['总数量'], sum1['总合计'], sum1['总毛重'], sum1['总件数']
...
def read_packing(sheet):
    packings, columns = read_sheet2(sheet, ('发票号', 'po号', '物料号'), ('数量', '净重', '毛重', '总数量', '总净重', '总毛重', '总件数', '总托盘数', '总箱数'))
...
def read_air(sheet):
    details, columns = read_sheet3(sheet, ('主运单号', '分运单号'), ('托盘数', '总毛重', '总件数'))
    all_sum = {
...
def check(proforma_invoice, packing_list, air_waybill):
    ...
    if packing_list is None:
        ...
        return
    file1, file2, file3 = load_workbook(proforma_invoice), load_workbook(packing_list), (
        load_workbook(air_waybill) if air_waybill else None)
    ...
    if keys1 == keys2:
        for key in keys1:
            ...
            if v1['sum']['总数量'] == v2['sum']['总数量']:
                logger.info('发票跟箱单：%s %s总数量相同', v1['sum']['发票号'],
